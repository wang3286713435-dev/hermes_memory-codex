...
11. Data Steward 能力必须先落到资产目录、派生索引、引用和人工审阅，再逐步接入本体、知识图谱、空间索引和子 Agent 编排。
...
### 5.15 Data Steward / BIM Asset Catalog Service

负责 Data Steward 后置产品线的资产目录治理，BIM 是首个高价值垂直场景。服务优先记录项目、楼栋、楼层、空间、专业、系统、模型类型、版本、原始路径 / 存储位置、文件大小、文件 hash、创建 / 修改时间、责任人、权限标签和派生元数据来源。该服务初期不搬迁原始 TB 级模型文件，不直接把原始模型内容送入 LLM 上下文，只管理可检索元数据、派生文本 / 清单 / 属性和可追溯链接，并为后续 IFC、RVT、NWD、DWG、点云、IoT 和运维系统 provider 预留接口。

### 5.16 Data Steward Planning Adapters

Data Steward 后置产品线可按以下适配器规划，但当前不实现新的 DB schema、Neo4j、PostGIS、空间索引代码或生产级 agent scheduler：

1. Asset Catalog Adapter：统一记录 BIM、图纸、表格、会议、合同、运维资料的资产、版本、路径、hash、责任人和权限。
2. Ontology Service：维护项目、楼栋、楼层、空间、构件、设备、传感器、系统、工单、合同和责任人的本体定义。
3. Graph Store Adapter：后续将派生实体和关系写成可审计三元组，例如“消防泵 P001 位于 B1 消防泵房”。
4. Spatial Index Adapter：后续保存构件、设备、空间和点云派生边界盒、坐标、楼层、区域索引，用于空间查询入口。
5. Agent Task Orchestrator：规划 Data Steward Supervisor、Catalog Agent、Ontology Agent、Spatial Index Agent、Monitoring Agent 和 Review Agent 的任务状态，不在当前阶段做自治调度。
6. Monitoring / Audit：记录 freshness、失败任务、权限拒绝、索引漂移、人工审阅和引用链路。
...
BIM / Data Steward 文件资产目录摄取可复用该流程的发现、hash、版本、权限、审计和索引能力，但初期应只生成资产目录记录和派生索引，不要求保存或解析完整原始模型内容。

Data Steward 后续摄取流程应保持只读优先：目录扫描 -> 资产记录 -> hash / version -> 权限标签 -> 派生元数据 -> 可引用索引 -> 数据质量问题 -> 人工审阅。任何真实数据修复、文件搬迁、模型重写、索引重建或自动运维动作都必须另开阶段并显式授权。
...
9. BIM / Data Steward：初期只接入资产目录和派生元数据；后续解析 provider 可逐步评估 IFC、RVT、NWD、DWG、点云、IoT 和运维系统数据，但不得把原始模型二进制或实时数据流直接作为 LLM 上下文。
...
BIM / Data Steward 资产目录应保留 asset_id、asset_type、project_id、building、floor、space、discipline、system、model_type、version、source_path、storage_location、file_size、content_hash、created_at、modified_at、owner、permission_tags 和 derived_metadata_source。模型构件级解析、几何解析、知识图谱写入、空间索引、碰撞检查、自动算量和在线查看器属于后续专项，不纳入当前技术设计落地范围。
