...
2. 当前唯一优先事项：Codex A 执行 `docs/NEXT_CODEX_A_PROMPT.md`，只做 Phase 2.45a docs-only Git baseline。
3. Phase 2.45a 只产出人工 runbook / checklist，未执行真实部署，未新增 deployment script。
4. Runbook 明确 Day-0 准备、目录创建、环境变量与 secrets、repo checkout、服务启动顺序、部署 / 热更新、minimum smoke、备份 / 回滚、stop conditions 与 operator sign-off。
5. 下一步只做 Phase 2.45a docs-only Git baseline；不得直接进入 Phase 2.45b 或实机部署。
...
7. Phase 2.45 结论：Mac mini 可作为 internal controlled MVP server 候选节点，但不是 production rollout；NAS 承载原始企业文件与未来 BIM 大文件，Mac mini 仅承载代码、热索引、缓存、日志与临时处理。
