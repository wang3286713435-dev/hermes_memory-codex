...
- [DB-2 Schema Handoff Freeze] 基于数据库 / NAS / 数字化交付平台侧第二轮确认，补齐 DB-2 到真实数据库前的 handoff freeze 文档：重写 `docs/DB2_SCHEMA_CONTRACT.md`，新增 `docs/DB2_VIEW_FIELD_MAPPING.md`、`docs/DB2_CHECKPOINT_AND_ROLLBACK_CONTRACT.md`、`docs/DB2_PERMISSION_DEFAULTS.md`、`docs/DB2_FAKE_FIXTURE_ACCEPTANCE_CASES.md`。验证通过：`npm test` 29 passed，`npm run lint` 通过，`git diff --check` 通过，QA probes 36 passed。本轮仍 docs-only，不写 migration、不连接真实 MySQL / NAS / REST、不写 documents/chunks/OpenSearch/Qdrant、不进入 DB-3 retrieval。
- [DB-3A Catalog Retrieval Guard] 新增 fake-preview-only catalog retrieval guard：授权项目可做 metadata lookup，但 `prompt_items` 永远为空；正文请求返回 Missing Evidence `asset_catalog_only`；缺 scope 返回 `permission_scope_required`；denied / moved / stale / missing / human-review rows 不进入 catalog result。本轮不接真实 MySQL / NAS / REST，不写 documents/chunks/OpenSearch/Qdrant，不创建 migration。
- [DB-3B Temporary DB Backed Guard] 新增 `AssetCatalogTemporaryMirrorStore.load_retrieval_preview()`，从 SQLite memory temporary mirror table 只读加载 catalog rows，并复用 DB-3A guard 执行权限过滤与 Missing Evidence；file-backed SQLite 继续拒绝。验证通过：`npm test` 37 passed，`npm run lint` 通过，py_compile 通过，`git diff --check` 通过。本轮仍不接真实 MySQL / NAS / REST，不写 migration，不写 documents/chunks/OpenSearch/Qdrant，不进入真实 retrieval/indexing。
- [DB-3C Missing Evidence Response DTO] 新增 `AssetCatalogMissingEvidenceResponse`，将 guard 的 Missing Evidence decision 转成稳定 response object，固定 `response_kind=missing_evidence`、reason、空 `prompt_items` 与 false write flags；覆盖 `asset_catalog_only`、`permission_scope_required`、`no_authorized_catalog_metadata`，并拒绝非 Missing Evidence catalog lookup decision。验证通过：`npm test` 43 passed，`npm run lint` 通过，py_compile 通过，`git diff --check` 通过。本轮仍不接真实 MySQL / NAS / REST，不写 migration，不写 documents/chunks/OpenSearch/Qdrant，不进入真实 retrieval/indexing。
- [DB-3C Review-fix] 修复 `DB3C-QA-001`：`AssetCatalogMissingEvidenceResponse.from_decision()` 现在拒绝 `None`、空字符串和空白字符串 reason。正式测试扩展为空/空白 reason case，QA probe `11 passed`，`npm test` 45 passed，lint / py_compile / diff check 通过。本轮仍不接真实 MySQL / NAS / REST，不写 migration，不写 documents/chunks/OpenSearch/Qdrant，不进入真实 retrieval/indexing。
- [DB-3D Temp DB Missing Evidence Response Smoke] 新增 `AssetCatalogMissingEvidenceResponse.from_preview()`，将 temporary DB backed preview 先交给 `AssetCatalogRetrievalGuard`，再包装为 Missing Evidence response。新增组合 smoke 覆盖 temp DB content answer -> `asset_catalog_only`、缺 scope -> `permission_scope_required`、授权 catalog lookup 不得包装成 Missing Evidence。验证通过：`npm test` 48 passed，`npm run lint` 通过，py_compile 通过，`git diff --check` 通过。本轮仍不接真实 MySQL / NAS / REST，不写 migration，不写 documents/chunks/OpenSearch/Qdrant，不进入真实 retrieval/indexing；DB-3D QA 通过后可考虑 DB-4A 只读 staging DB preflight。
- [DB-4A Readonly DB Preflight Skeleton] 基于数据库团队 `enterprise-agent-db-connection-contract.md` 新增只读预检骨架：`AssetCatalogReadonlyPreflightValidator` 只接收内存中的四个 View rows，校验字段清单和 unsupported View，归一化为 `AssetCatalogMirrorPreview`，并在权限字段缺失时 fail closed 为 `would_deny` / `missing_permission_contract`。新增只读 DB 配置默认 off：`platform_asset_readonly_db_enabled=false`、DSN/user 为空、contract version 为 `delivery_platform.asset_views.v1`。验证通过：`npm test` 53 passed，`npm run lint` 通过，py_compile 通过，`git diff --check` 通过。本轮仍不连接真实 MySQL、不写 migration、不扫 NAS、不触发 REST、不写 documents/chunks/OpenSearch/Qdrant、不进入真实 retrieval/indexing。
- [DB-4B Readonly Connector Shell] 基于 `enterprise-agent-db2-coupling-handoff.md` 新增 disabled-by-default readonly connector shell：默认只生成 `WHERE 1 = 0` 字段握手 SQL，显式 `limit` 模式最多 `LIMIT 30`，只允许四个稳定 View，并把 fake/DB-API rows 交给 DB-4A preflight validator。新增 password/sample mode/sample limit 配置默认值。验证通过：`npm test` 60 passed，`npm run lint` 通过，py_compile 通过，`git diff --check` 通过。本轮仍不 import MySQL driver、不连接真实 MySQL、不写 migration、不扫 NAS、不触发 REST、不写 documents/chunks/OpenSearch/Qdrant、不进入真实 retrieval/indexing。
- [DB-4C Readonly Live Smoke Runner] 新增 live smoke runner 关闭态骨架：`structure_only` 模式只读取 cursor column description 做 required-column validation，不读取真实 rows；`limit` 真实样本模式必须同时满足 `mainline_agent_updated=true` 与 `allow_real_sample_data=true`，对应 DB 分支回归主线并更新数据库团队电脑企业 Agent 版本后的显式授权门槛。验证通过：`npm test` 66 passed，`npm run lint` 通过，py_compile 通过，`git diff --check` 通过。本轮仍不连接真实 MySQL、不读取真实样本、不写 migration、不扫 NAS、不触发 REST、不写 documents/chunks/OpenSearch/Qdrant、不进入真实 retrieval/indexing。
- [DB-4D Readonly Local Live Smoke] 新增测试机同机 dev 只读 smoke 执行层：`DockerMysqlReadonlyQueryRunner` 通过 Docker MySQL CLI 查询 `delivery-mysql`，密码只从进程环境变量读取且不进入 argv；`run_readonly_local_live_smoke()` 复用 DB-4A/B/C 并只输出脱敏 JSON summary；新增同机本地 dev 授权门禁 `same_machine_local_dev_authorized`。用户已澄清当前开发机无真实数据库环境，真实 DB smoke 等 Hermes Memory 安装到测试机后执行；本轮只保留耦合接口，未读取真实 rows。验证通过：`npm test` 71 passed，`npm run lint` 通过，py_compile 通过，`git diff --check` 通过。
- [DB Branch Closeout] DB-4D 独立 QA 返回 `DB4D_QA_OPEN_FINDINGS: 0`，P0=0；新增 `docs/DB_BRANCH_CLOSEOUT_AND_MERGE_READINESS.md`，将 DB-0 到 DB-4D 完成范围、未做范围、feature flag 默认安全态、合回主线条件和测试机真实 DB smoke 后置门槛固化。当前建议停止扩大 DB 功能，进入 closeout / merge readiness。
